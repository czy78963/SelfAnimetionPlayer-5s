...
- (void)loadSignInControls {
    //头像
//    UILabel *labelForHead = [[UILabel alloc] initWithFrame:CGRectMake(10, 70, 100, 20)];
//    labelForHead.text = @"设置头像:";
//    labelForHead.textColor = [UIColor colorWithRed:0.3862 green:0.7749 blue:0.4809 alpha:1.0];
//    labelForHead.font = [UIFont systemFontOfSize:14];
//    [self.view addSubview:labelForHead];
...
//    UITapGestureRecognizer *tapGes = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapChooseImage:)];
//    //为控件添加手势控制器
...
////单击手势
//- (void)tapChooseImage:(UITapGestureRecognizer *)gesture {
//    NSLog(@"选择头像");
    ...
    if ([_setAccountTextView.text isEqual: @""] || [_setPasWordTextView.text isEqual:@""]|| [_surePasWordTextView.text isEqual:@""] || [_setNicoNameTextView.text isEqual:@""]) {
        ...
        [user signUpInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
            if (succeeded) {
                ...
                [dic setObject:_setAccountTextView.text forKey:@"account"]; //账号
                [dic setObject:_setPasWordTextView.text forKey:@"password"];//密码
                [dic setObject:_setNicoNameTextView.text forKey:@"niconame"];//昵称
                [[NSUserDefaults standardUserDefaults] setObject:dic forKey:_setAccountTextView.text];
                [[NSUserDefaults standardUserDefaults] synchronize];
                
                NSString *strKeyNew = [NSString stringWithFormat:@"coll%@",_setAccountTextView.text];
                if (![[NSUserDefaults standardUserDefaults] objectForKey:strKeyNew]) {
                    [[NSUserDefaults standardUserDefaults] setObject:[NSArray array] forKey:strKeyNew];
                    [[NSUserDefaults standardUserDefaults] synchronize];
                }
